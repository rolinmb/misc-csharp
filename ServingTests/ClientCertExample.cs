...
namespace Client{
    class Program{
		...
		public static void Main(string[] args){
			...
			X509Certificate2 cert = new X509Certificate2("C:\\ssl\\certs\\test.cert.pem");
			string data = "Private Stuff! This has to be encrypted and then decrypted!";
			Console.WriteLine("Unencrypted Data: "+data);
			RSA crypto = cert.GetRSAPublicKey();
			byte[] toEncrypt = ASCIIEncoding.ASCII.GetBytes(data);
			byte[] encryptedBytes = crypto.Encrypt(toEncrypt,RSAEncryptionPadding.OaepSHA1);
			string encryptData = Convert.ToBase64String(encryptedBytes);
			Console.WriteLine("\nEncrypted Data: "+encryptData);
			byte[] decryptedBytes = crypto.Decrypt(encryptedBytes,RSAEncryptionPadding.OaepSHA1);
